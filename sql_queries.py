...
time_table_create = ("""
    ...
    month INTEGER,
    year INTEGER ENCODE BYTEDICT,
    weekdat VARCHAR(9) ENCODE BYTEDICT
    )
    DISTSTYLE KEY
    DISTKEY (start_time)
    SORTKEY (start_time);
...
user_table_insert = ("""
    INSERT INTO users
    SELECT DISTINCT userId, userName, lastName, gendder, level
    FROM staging_events
